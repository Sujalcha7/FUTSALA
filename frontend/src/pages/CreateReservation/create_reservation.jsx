...
const Calendar = ({ selectedDate, setSelectedDateAndUpdateRange }) => {
  ...
  const handleMonthChange = (direction) => {
    const maxDate = dayjs().add(2, "week").endOf("day");
    const newDate = selectedDate.add(direction, "month");

    if (newDate.isAfter(maxDate, "day")) {
      return; // Prevent changing to a month beyond the allowed range
    }
      ...
      <CardBody>
        <Grid templateColumns="repeat(7, 1fr)" gap={2} mb={4}>
          {daysOfWeek.map((day, index) => (
            <GridItem key={index} textAlign="center" color="gray.400">
              {day}
            </GridItem>
          ))}
        </Grid>

        <Grid templateColumns="repeat(7, 1fr)" gap={2} placeItems="center">
          ...
          {daysInMonth.map((day) => (
            <GridItem
              ...
              textAlign="center"
              cursor="pointer"
              bg={selectedDate.date() === day ? "#2c698d" : "#ffffff"}
              ...
              _hover={{
                bg: selectedDate.date() === day ? "#2c698d" : "#bae8e8",
              }}
              aspectRatio={1}
            >
              {day}
...
const TimeSelector = ({
  ...
  const { isOpen: isStartTimeOpen, onToggle: onStartTimeToggle } =
    useDisclosure();
  const { isOpen: isEndTimeOpen, onToggle: onEndTimeToggle } = useDisclosure();
  ...
  const isTimeSlotReserved = (hour) => {
    ...
    const isPastDay = selectedDate.isBefore(today, "day");
    return (
      isPastDay ||
      hour - 1 < currentHour ||
      alreadyReservedRange.some(
                  ...
                  <PopoverTrigger>
                    ...
                    >
                      {selectedStartTime !== null
                        ? dayjs().hour(selectedStartTime).format("hh:00 A")
                        : "Start with"}
                    </Button>
                  </PopoverTrigger>
                        ...
                        <VStack align="stretch" spacing={2}>
                          {timeSlots.map((slot) => (
                            <Radio
                              key={slot.hour}
                              value={slot.hour}
                              isDisabled={isTimeSlotReserved(slot.hour)}
                              onChange={() => handleTimeSelect(slot.hour, true)}
                              colorScheme="red"
                            >
                              <Text color="white">{slot.display}</Text>
                            </Radio>
                          ))}
                        </VStack>
                      </RadioGroup>
                  ...
                  <PopoverTrigger>
                    ...
                    >
                      {selectedEndTime !== null
                        ? dayjs().hour(selectedEndTime).format("hh:00 A")
                        : "End with"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent bg="gray.700">
                        ...
                        <VStack align="stretch" spacing={2}>
                          {timeSlots.map((slot) => (
                            <Radio
                              ...
                              onChange={() =>
                                handleTimeSelect(slot.hour, false)
                              }
                              colorScheme="red"
                            >
                              <Text color="white">{slot.display}</Text>
                  ...
                  >
                    <VStack spacing={3} align="stretch">
                      <Flex justify="space-between" align="center">
                        <Text color="#272643" fontWeight="medium">
                          Selected Range:
                        </Text>
                      </Flex>
                      <Flex justify="space-between" align="center">
                        <Text color="#272643">Date: </Text>
                        <Text>{selectedDate.format("DD.MM.YYYY")}</Text>
                      </Flex>
                      <Flex justify="space-between" align="center">
                        <Text color="#272643">Time: </Text>
                        <Text>
                          {selectedStartTime !== null &&
                          selectedEndTime !== null
                            ? `${dayjs()
                                .hour(selectedStartTime)
                                .format("hh:00 A")} - ${dayjs()
          ...
          <GridItem>
            <VStack spacing={4} align="stretch" h="100%">
              ...
              >
                <VStack spacing={3} align="stretch">
                  <Flex justify="space-between" align="center">
                    <Text color="#272643">Total Duration:</Text>
                    <Text color="#272643">{totalDuration} hours</Text>
                  </Flex>
                  <Flex justify="space-between" align="center">
                    <Text color="#272643">Rate per hour:</Text>
                    <Text color="#272643">Rs {ratePerHour}</Text>
                  </Flex>
                  <Flex justify="space-between" align="center">
      ...
      await Promise.all(
        selectedRanges.map(async ([date, ranges]) => {
          for (const { start, end } of ranges) {
            ...
            const endDateTime = dayjs(date).hour(end).startOf("hour").format(); // Remove .utc() to preserve local time

            await axios.post("http://localhost:8000/api/create_reservation/", {
      ...
      const formattedSelections = selectedRanges
        .map(([date, ranges]) => {
          const formattedRanges = ranges
            .map(
              ({ start, end }) =>
                `${String(start).padStart(2, "0")}:00 - ${String(
                  end + 1
                ).padStart(2, "0")}:00`
            )
            .join(", ");
